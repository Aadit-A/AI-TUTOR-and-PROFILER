...
const CMP: Record<string, (a: string, b: string) => boolean> = {
  ...
  sorted: (a, b) => {
    try {
      return JSON.stringify(JSON.parse(a).sort()) === JSON.stringify(JSON.parse(b).sort());
    } catch {
...
async function judge(problemId: number, userCode: string) {
  await connectDB();
  ...
  if (exec.compileErr) {
    return {
      ...
      error: exec.stderr,
      passed: 0,
      total: tc.tests.length,
      score: 0,
      results: []
    };
  }

  if (exec.runtimeErr && !exec.stdout) {
    return {
