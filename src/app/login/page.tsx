...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex bg-[#f3f6f4] text-[#18292c]">
      {/* Left - Branding */}
      <div className="relative hidden w-1/2 items-center justify-center bg-[#18292c] p-12 lg:flex">
        <div className="absolute inset-0 bg-gradient-to-br from-teal-500/20 to-cyan-500/20" />
        <div className="relative z-10 max-w-lg text-center">
          <div className="mx-auto mb-8 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-teal-500 to-cyan-600"><Brain size={32} className="text-white" /></div>
          <h2 className="text-4xl font-bold text-white mb-6">Welcome Back</h2>
          <p className="text-lg text-[#b9cecd]">Practice coding with AI-powered guidance and track your progress.</p>
        </div>
      </div>

      {/* Right - Form */}
      <div className="flex w-full items-center justify-center bg-[#edf3f0] p-8 lg:w-1/2">
        <div className="w-full max-w-md rounded-2xl border border-[#d6e2de] bg-white p-10 shadow-lg">
          <div className="text-center mb-8">
            <h2 className="text-2xl font-bold text-[#18292c]">Sign in to your account</h2>
            <p className="mt-2 text-sm text-[#5a7275]">Don&apos;t have an account? <Link href="/register" className="text-teal-700 hover:text-teal-600">Sign up</Link></p>
          </div>
          ...
          <button onClick={handleGoogle} disabled={loading} className="flex w-full items-center justify-center gap-3 rounded-lg border border-[#cfdcd8] py-3 font-medium transition-colors hover:bg-[#edf3f0] disabled:opacity-50">
